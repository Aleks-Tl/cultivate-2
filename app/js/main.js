    ...
    jQuery(function ($) {
      $('.filter__select').each(function () {
        ...
        $listItems.click(function (e) {
          ...
          console.log($('#select-duration').val());
          let params = $(this).parents('form').siblings('.classes__filtered');
          let nameOption = $(this).text();
          let valOption = $(this).attr('rel');
          if ($(this).parent().siblings('select').attr('id') === 'select-duration') {
            if (params.find('.option-duration').length > 0) {
              params.find('.option-duration').text(nameOption + ' ×');
            } else {
              params.append('<span class="option-duration">' + nameOption + ' ×</span>');
            }
            if (valOption === 'default') {
              $('.option-duration').remove();
            }
          }
          if ($(this).parent().siblings('select').attr('id') === 'select-intensity') {
            if (params.find('.option-intensity').length > 0) {
              params.find('.option-intensity').text(nameOption + ' ×');
            } else {
              params.append('<span class="option-intensity">' + nameOption + ' ×</span>');
            }
            if (valOption === 'default') {
              $('.option-intensity').remove();
            }
          }
          if ($(this).parent().siblings('select').attr('id') === 'select-style') {
            if (params.find('.option-style').length > 0) {
              params.find('.option-style').text(nameOption + ' ×');
            } else {
              params.append('<span class="option-style">' + nameOption + ' ×</span>');
            }
            if (valOption === 'default') {
              $('.option-style').remove();
            }
          ...
          if ($('#filter-two').length) {
            var filterTwo = $('#filter-two');
            $.ajax({
              url: myajax.url, // обработчик
              data: {
                action: 'my_action',
                filterData: filterTwo.serializeArray()
              },
              type: 'POST', // тип запроса
              success: function (data) {
                $('#response-two').html(data);
              }
            });
          }
      ...
      $('body').on('click', '.classes__filtered span', function (e) {
        if ($(this).hasClass('option-duration')) {
          let defaultVal = $(this).parents('.classes__filtered').siblings('form').find('#select-duration option.select__current').text();
          console.log(defaultVal);
          $(this).parents('.classes__filtered').siblings('form').find('select#select-duration').siblings('.select-styled').text(defaultVal);
          $(this).parents('.classes__filtered').siblings('form').find('select#select-duration').val('default');
        }

        if ($(this).hasClass('option-intensity')) {
          let defaultVal = $(this).parents('.classes__filtered').siblings('form').find('#select-intensity option.select__current').text();
          $(this).parents('.classes__filtered').siblings('form').find('select#select-intensity').siblings('.select-styled').text(defaultVal);
          $(this).parents('.classes__filtered').siblings('form').find('select#select-intensity').val('default');
        }

        if ($(this).hasClass('option-style')) {
          let defaultVal = $(this).parents('.classes__filtered').siblings('form').find('#select-style option.select__current').text();
          $(this).parents('.classes__filtered').siblings('form').find('select#select-style').siblings('.select-styled').text(defaultVal);
          $(this).parents('.classes__filtered').siblings('form').find('select#select-style').val('default');
        }
        console.log($('#select-duration').val());

        var filter = $('#filter');
        var filterTwo = $('#filter-two');
        if ($(this).parents('.classes__filtered').siblings('form').attr('id') === 'filter') {
          $.ajax({
            url: myajax.url, // обработчик
            data: {
              action: 'my_action',
        ...
        } else if ($(this).parents('.classes__filtered').siblings('form').attr('id') === 'filter-two') {
          $.ajax({
            url: myajax.url, // обработчик
            data: {
              action: 'my_action',
